...
export const WebSocketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    const overrideCandidate = (() => {
      ...
      if (!raw) return null;
      if (typeof raw === 'string') return { origin: raw } as { origin: string };
      if (typeof raw === 'object') return raw as { origin?: string; protocol?: string; hostname?: string; port?: string | number };
      return null;
    })();
