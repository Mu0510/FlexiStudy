...
class BackgroundGemini {
  ...
  async promptText(promptText, { timeoutMs = 45000 } = {}) {
    return new Promise((resolve, reject) => {
      this.queue.push({ promptText, timeoutMs, resolve, reject });
      this.drain();
    });
  }
  ...
  async executePrompt(promptText, timeoutMs) {
  ...
  drain() {
    ...
    this.running = true;
    this.executePrompt(next.promptText, next.timeoutMs)
      .then((value) => {
        try {
          next.resolve(value);
        } catch {}
