// Dashboard specific JavaScript
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const chatParam = getUrlParameter('chat');

            // chat.jsがロードされるのを待ってからボタンをクリック
            const checkChatJsLoaded = setInterval(() => {
                if (window.chatInitialized) { // chat.jsで設定されるフラグ
                    clearInterval(checkChatJsLoaded);
                    if (chatParam === 'fullscreen') {
                        const fullscreenButton = document.getElementById('fullscreenToggle');
                        if (fullscreenButton) {
                            fullscreenButton.click();
                        }
                    } else if (chatParam === 'hidden') {
